...
export class ClassComponentParser extends ScriptParser {
    constructor(parent, root, source, file, options) {
    ...
    parsePropDecorator(node, [argument], type, name) {
        if (this.features.includes(Feature.props)) {
            const prop = {
                type,
                required: false,
    ...
    parsePropSyncDecorator(node, [propNameArg, propObjectArg], type, computedName) {
    ...
    parseModelDecorator(node, [eventNameArg, propObjectArg], type, propName) {
        if (this.features.includes(Feature.props)) {
    ...
    parseModelSyncDecorator(node, args, type) {
    ...
    parseModelVModelDecorator(node, [propObjectArgNode], type) {
    ...
    parseRefDecorator(node, args, type) {
    ...
    parseStateDecorator(node, args, type) {
        if (this.features.includes(Feature.data)) {
            new ClassComponentDataParser(this.root, this.emitter, this.source, this.scope).parse(node, type);
        }
    }
    parseEmitDecorator(node, [eventNameArg], methodName) {
        const { params } = node;
...
//# sourceMappingURL=ClassComponentParser.js.map