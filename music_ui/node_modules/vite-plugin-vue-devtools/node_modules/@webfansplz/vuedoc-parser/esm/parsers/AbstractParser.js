...
function copyComments(dest, source) {
...
export class AbstractParser {
    constructor(root, emitter, source, scope) {
        this.root = root;
    ...
    parseFunctionDeclaration(_node) { }
    /* istanbul ignore next */
    /* eslint-disable no-unused-vars */
    /* eslint-disable class-methods-use-this */
    parseEntryComment(_entry, _node) { }
    /* istanbul ignore next */
    /* eslint-disable no-unused-vars */
    /* eslint-disable class-methods-use-this */
    parseArrayExpression(_node) { }
    /* istanbul ignore next */
    /* eslint-disable no-unused-vars */
    /* eslint-disable class-methods-use-this */
    parseCallExpression(_node) { }
    /* istanbul ignore next */
    /* eslint-disable no-unused-vars */
    /* eslint-disable class-methods-use-this */
    parseFunctionExpression(_node) { }
    /* istanbul ignore next */
    /* eslint-disable no-unused-vars */
    /* eslint-disable class-methods-use-this */
    parseExpressionStatement(_node) { }
    /* istanbul ignore next */
    /* eslint-disable no-unused-vars */
    /* eslint-disable class-methods-use-this */
    parseObjectExpression(_node) { }
    ...
    parseVariableDeclarator(declarator, node) {
        if (declarator.init) {
    ...
    parseVariableDeclarationArrayPattern(declarator, id) {
    ...
    parseVariableDeclarationObjectPattern(declarator, id) {
    ...
    getVariableDeclarationElementIdentifier(key, options, defaultValue) {
        const decoratorValue = this.getDeclaratorValue({
            init: options.init,
            id: options.id,
        });
        ...
        for (let index = 0; index < elements.length; index++) {
            ...
            switch (element.type) {
                ...
                case Syntax.ArrayPattern: {
                    ...
                    const node = tsValue.type === Type.array ? tsValue.node.elements[index] : {
                ...
                case Syntax.ObjectProperty: {
                    ...
                    switch (element.value.type) {
                        ...
                        case Syntax.ObjectPattern: {
                            ...
                            const properties = 'type' in tsValue.node && tsValue.node.type === Syntax.ObjectExpression
                                ...
                                : [];
                            const node = properties.find((property) => this.parseKey(property) === source) || null;
                    ...
                    if (name) {
                        switch (options.init.type) {
                            ...
                            case Syntax.ObjectExpression: {
                                const propertyValue = options.init.properties.find((item) => {
    ...
    parseVariableDeclarationElementsRest(element, options) {
    ...
    setScopeValue(key, node, value, { source = undefined, nodeComment = node, nodeTyping = node, isFunction = false, forceType = false, global = false, tsValue = undefined, composition = undefined, } = {}) {
        if (!forceType) {
        ...
        if (global && this.root) {
            this.root.scope[key] = this.scope[key];
        }
    }
    getScopeValue(key) {
        if (key in this.scope) {
    ...
    setLeftSidePart(node, parent, declarator) {
        node.extra[LEFT_SIDEPART_KEY] = { declarator, parent };
    }
    getLeftSidePart(node, defaultResult = null) {
        return node.extra[LEFT_SIDEPART_KEY] || defaultResult;
    }
    hasLeftSidePart(node) {
        return LEFT_SIDEPART_KEY in node.extra;
    }
    getReturnType(node, defaultType = Type.unknown) {
        ...
        if (_node.returnType) {
            ...
            type = _node.typeAnnotation.typeAnnotation.type === Syntax.TSFunctionType
                ? this.getTSValue(_node.typeAnnotation.typeAnnotation.typeAnnotation).type
                : this.getTSValue(_node.typeAnnotation.typeAnnotation).type;
        }
        else if (AbstractParser.isFunction(_node)) {
            const blockNode = _node.body?.body || _node.body;
            const returnNode = blockNode instanceof Array
    ...
    getCompositionReturnType(argument, options) {
        if ('typeAnnotation' in options.id && options.id.typeAnnotation) {
            return this.getTSValue(options.id.typeAnnotation).type;
        }
        if ('returnType' in argument) {
            return this.getTSValue(argument.returnType).type;
        }
        return this.getReturnType(argument, Type.unknown);
    }
    parseCompositionTypeParameters(init) {
    ...
    getCompositionValue(options) {
        ...
        const init = options.init.type === Syntax.TSAsExpression
        ...
        if (init.type === Syntax.CallExpression && 'callee' in init && 'name' in init.callee) {
            ...
            if (composition) {
                ...
                if ('parseEntryValue' in composition && typeof composition.parseEntryValue === 'function') {
                    ref = composition.parseEntryValue(init, this);
                }
    ...
    getTSTypeLiteralValue(node) {
        const type = {};
        const members = {};
        for (const member of node.members) {
            const key = this.parseKey(member);
            const ts = this.getTSValue(member);
            const ref = this.getScopeValue(key);
            if (ref) {
                ref.node.comment = member;
            }
            member.extra.computed = ts.computed || member.computed;
            members[key] = member;
    ...
    getTSInterfaceBody(node) {
        const ref = {};
        for (const item of node.body) {
            const key = this.parseKey(item);
            const tsValue = this.getTSValue(item);
            ref[key] = tsValue;
        }
        return ref;
    }
    getTSValue(node, force = false) {
        if (node.extra.$tsvalue && !force) {
            return node.extra.$tsvalue;
        }
        switch (node.type) {
            case Syntax.TSAsExpression:
            case Syntax.TSTypeAnnotation:
            ...
            switch (item.type) {
                case Syntax.ObjectProperty: {
                    ...
                    const refNode = item.value.type === Syntax.Identifier && item.value.name in this.scope && 'node' in this.scope[item.value.name]
                        ? this.scope[item.value.name].node.value
                        : item.value;
                ...
                default: {
                    ...
                    const refNode = item.type === Syntax.Identifier && item.name in this.scope && 'node' in this.scope[item.name]
                        ? this.scope[item.name].node.value
                        : item;
                    object.value[item.key.name] = ref.value;
                    object.rawObject[item.key.name] = ref;
                    object.rawNode[item.key.name] = refNode;
    ...
    getSourceString(node) {
        const source = node.extra.file?.script?.content || this.source.content;
    ...
    getMemberExpression(node) {
        ...
        if (type === Type.object) {
            switch (node.object.type) {
                case Syntax.Identifier: {
                    const key = 'name' in node.property ? node.property.name : this.getValue(node.property).value;
    ...
    getCallExpression(node) {
        const type = node.callee.type === Syntax.Identifier && TypeList.includes(node.callee.name.toLowerCase())
            ? node.callee.name
            : Type.unknown;
        return this.getSourceValue(node, type);
    }
